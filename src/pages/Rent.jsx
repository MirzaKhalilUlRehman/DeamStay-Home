...
export default function Rent() {
  ...
  // Initial filter for rent-only items
  const baseRentEstates = properties.filter(p => p.purpose === 'rent');

  // Dynamic real-time filtration
  const filteredRentEstates = baseRentEstates.filter(prop => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 text-center sm:text-left">
          ...
          <p className="text-stone-300 text-xs mt-1">
            Browse our premium listings of luxury houses, villas, lofts, and beachfront penthouses available for rent.
          </p>
        </div>
        ...
        <div className="bg-[#090b1c]/60 border border-purple-500/20 p-5 rounded-3xl mb-12 shadow-2xl backdrop-blur-md">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
            ...
            <div className="flex flex-col gap-1.5 focus:border-purple-500">
              ...
              <div className="flex items-center gap-2 bg-[#05060f] border border-purple-500/15 p-3 rounded-xl">
                ...
                <input 
                  type="text"
                  placeholder="Filter by location (e.g. Malibu)..."
                  value={location}
          ...
          <div className="mt-4 flex justify-between items-center text-[10px] text-stone-500 font-mono">
            ...
            <span className="text-purple-400 font-bold">{filteredRentEstates.length} matches detected</span>
          </div>
        </div>

        {/* Listings Display Grid */}
        {filteredRentEstates.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredRentEstates.map(p => (
          ...
          <div className="glass-panel p-16 rounded-3xl text-center flex flex-col items-center">
            ...
            <h3 className="font-serif text-lg font-bold text-white">No Rentals Found</h3>
            <p className="text-stone-400 text-xs mt-2 max-w-xs font-sans font-light">
              We couldn't find any rentals matching your current search options. Try widening your price range or clearing filters.
            </p>
          </div>
        )}
